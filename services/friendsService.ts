...
export async function sendFriendInvite(username: string) {
    return await useApi<ApiResponse<{ invite: InviteResponse }>>(
        'POST',
        '/api/friends/invite',
        {
            username,
...
export async function fetchInvites() {
    return await useApi<ApiResponse<InvitesResponse>>(
        'GET',
        '/api/friends/invites',
    );
}
...
export async function rejectInvite(inviteId: string) {
    return await useApi<
        ApiResponse<{ invite: InvitesResponse['sentInvites'][0] }>
    >('PATCH', `/api/friends/invites/${inviteId}/reject`);
}

export async function deleteFriend(friendId: string) {
    return await useApi<ApiResponse<DeleteFriendResponse>>(
        'DELETE',
