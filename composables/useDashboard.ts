...
export function useDashboard() {
    ...
    const typingUsersComposable = useTypingUsers(
    ...
    const typingUsersByChat = computed(() => {
        return typingUsersComposable.updateTypingUsersByChat(
            chatsComposable.chats.value,
        );
    });
    ...
    function handleChatUpdated(data: {
        members: any[];
        currentUserRole?: any;
    }) {
    ...
    async function handleMarkLatestMessageAsRead() {
        ...
        const nonSystemMessages = chat.messages.filter(
            (msg) =>
                !msg.isSystem && !compareIds(msg.senderId, currentUserId.value),
        );
    ...
    function cleanup() {
        window.removeEventListener('scroll-to-message', ((
            event: CustomEvent,
        ) => {
            handleMessageSearchScroll(event.detail.messageId);
        }) as EventListener);
