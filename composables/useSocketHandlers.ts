...
export function useSocketHandlers(
    ...
    messageReads: ReturnType<
        typeof import('./useMessageReads').useMessageReads
    >,
    ...
    function handleNewMessage(data: { chatId: string; message: any }) {
        ...
        if (wasAdded) {
            if (
                ...
                !compareIds(selectedChatId.value, chat.id)
            ) {
    ...
    function handleChatUpdatedFromSocket(data: {
        chatId: string;
        updates: any;
    }) {
        const chatId = String(data.chatId);
        ...
        if (messageIndex !== -1) {
            ...
            if (pinnedMessageData?.pinnedAt) {
                if (typeof pinnedMessageData.pinnedAt === 'string') {
                    ...
                    pinnedAtValue = (
                        pinnedMessageData.pinnedAt as Date
                    ).toISOString();
                } else {
    ...
    function handleMessageRead(data: {
        ...
        const chatId = String(data.chatId);
        const readerUserId =
            typeof data.reader.userId === 'string' &&
            isNaN(Number(data.reader.userId))
                ? data.reader.userId
    ...
    function handleFriendInviteAccepted(data: FriendInviteAcceptedEvent) {
        friendsComposable.addFriendFromEvent(
            data.friendship,
            currentUserId.value,
        );
