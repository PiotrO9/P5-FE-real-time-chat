...
export function useChats() {
    ...
    async function fetchChats() {
        try {
            ...
            const res =
                (await fetchChatsFromService()) as ApiResponse<ChatsResponse>;
            const raw = res?.data;
            ...
            chats.value = list.map((chat) => ({
                ...
                unreadCount: Number.isFinite(chat?.unreadCount)
                ...
                members: Array.isArray(chat?.members)
                    ...
                    : undefined,
                currentUserRole:
                    chat?.currentUserRole || chat?.memberRole || undefined,
                hasOnlineMembers: chat?.hasOnlineMembers ?? false,
            }));
        } catch (err: any) {
            chatsError.value = getErrorMessage(
                err,
                'Failed to fetch chats list',
            );
