...
export function useInvites() {
    ...
    async function fetchInvites() {
        try {
            ...
            invites.value = {
                sentInvites: Array.isArray(raw?.sentInvites)
                    ? raw.sentInvites
                    : [],
                receivedInvites: Array.isArray(raw?.receivedInvites)
                    ? raw.receivedInvites
                    : [],
            ...
            invitesError.value = getErrorMessage(
                err,
    ...
    function addReceivedInvite(
        inviteData: FriendInviteReceivedEvent['invite'],
    ) {
        const invite: Invite = {
            id: inviteData.id,
            ...
            receiver: inviteData.receiver
                ? mapUserToInviteUser(inviteData.receiver)
                : undefined,
        };

        const existingIndex = findIndexById(
            invites.value.receivedInvites,
            invite.id,
        );
        if (existingIndex === -1) {
    ...
    function removeInvite(inviteId: string) {
        ...
        const receivedIndex = findIndexById(
            invites.value.receivedInvites,
            inviteId,
        );
        if (receivedIndex !== -1) {
    ...
    function updateInviteStatus(
        inviteData: FriendInviteRejectedEvent['invite'],
    ) {
        ...
        if (receivedIndex !== -1) {
            ...
            invites.value.receivedInvites[receivedIndex] = {
                ...
                sender: inviteData.sender
                    ? mapUserToInviteUser(inviteData.sender)
                    : undefined,
            };
        }
    }

    function removeInvitesByUserIds(
